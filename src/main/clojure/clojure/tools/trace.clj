...
may be rebound to do anything you like. 'name' is optional."
  [name value]
  (print (str "TRACE" (when name (str " " name)) ": " value)))
...
affecting the result."
  ...
  ([name value]
     (tracer name (with-out-str (aprint value)))
     value))
...
symbol name of the function."
  [name f args]
  (let [id (gensym "t")]
    (tracer id (str (trace-indent) (with-out-str (aprint (cons name args)))))
    (let [value (binding [*trace-depth* (inc *trace-depth*)]
                  (apply f args))]
      (tracer id (str (trace-indent) "=> " (with-out-str (aprint value))))
      value)))
